...
function eliminar(){
    ...
    for(i=0; i<eliminar.length ; i++){
        eliminar[i].onclick = function(){
            ...
            tareaAEliminar = tareas.filter(t => t.id == li.getAttribute('data-tarea-id'))[0];
            tareas = tareas.filter(t => t.id != li.getAttribute('data-tarea-id'));
            
            fetch('http://localhost:5000/api/tasks/'+tareaAEliminar._id, {
                method: 'DELETE',
                headers: {
...
function finalizar(){
    ...
    for(i=0; i<finalizar.length ; i++){
        finalizar[i].onclick = function(){
            ...
            const tareaAActualizar = tareas[tareas.findIndex(t => t.id == li.getAttribute('data-tarea-id'))];
            
            
            
            fetch('http://localhost:5000/api/tasks/'+tareaAActualizar._id, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json'
                },
                body: JSON.stringify(tareaAActualizar)
            })
            .then(response => {
                return response.json( )
            })
